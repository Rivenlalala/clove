...
class TestEmitLogFormat(unittest.TestCase):
    ...
    def test_emit_log_correct_format_all_fields(self):
        ...
        context = _make_context(
            messages_api_request=_make_request(model="claude-sonnet-4-20250514", stream=True),
            collected_message=_make_collected_message(input_tokens=1234, output_tokens=567),
        )
    ...
    def test_emit_log_oauth_cache_read_from_sse_usage(self):
        context = _make_context()
        oauth_usage = {"input_tokens": 100, "output_tokens": 50, "cache_read_input_tokens": 80, "cache_creation_input_tokens": 0}
    ...
    def test_emit_log_oauth_cache_write_from_sse_usage(self):
        context = _make_context()
        oauth_usage = {"input_tokens": 100, "output_tokens": 50, "cache_read_input_tokens": 0, "cache_creation_input_tokens": 200}
    ...
    def test_emit_log_oauth_no_cache_defaults_false(self):
        context = _make_context()
        oauth_usage = {"input_tokens": 100, "output_tokens": 50, "cache_read_input_tokens": 0, "cache_creation_input_tokens": 0}
    ...
    def test_emit_log_oauth_usage_overrides_collected_message(self):
        context = _make_context(
            collected_message=_make_collected_message(input_tokens=100, output_tokens=50),
        )

        with patch(PATCH_LOGGER) as mock_logger:
            self.processor._emit_log(context, oauth_usage={"input_tokens": 999, "output_tokens": 888})
...
class TestProcessMethod(unittest.TestCase):
    ...
    def test_process_streaming_full_lifecycle_oauth(self):
        start = _make_sse_bytes("message_start", {
            "type": "message_start",
            "message": {"id": "msg_1", "usage": {"input_tokens": 100, "output_tokens": 1}},
        })
        delta = _make_sse_bytes("message_delta", {
            "type": "message_delta",
        ...
        mock_logger.info.assert_called_once()
        pairs = dict(p.split("=", 1) for p in mock_logger.info.call_args[0][0].split(" "))
    ...
    def test_process_streaming_full_lifecycle_web_proxy(self):
        ...
        context = _make_context(
            messages_api_request=_make_request(stream=True),
            response=StreamingResponse(_iter()),
            collected_message=_make_collected_message(input_tokens=500, output_tokens=200),
        )
        context.metadata["request_start_time"] = time.monotonic()
...
class TestWrapStream(unittest.TestCase):
    ...
    def test_wrap_stream_yields_all_chunks_unchanged_oauth(self):
        ...
        with patch(PATCH_LOGGER):
            result = asyncio.run(_collect_stream(self.processor._wrap_stream(_iter(), context)))
    ...
    def test_wrap_stream_emits_log_on_completion_oauth(self):
        start = _make_sse_bytes("message_start", {
            "type": "message_start",
            "message": {"id": "msg_1", "usage": {"input_tokens": 200, "output_tokens": 1}},
        })
        delta = _make_sse_bytes("message_delta", {
            "type": "message_delta",
            "delta": {"stop_reason": "end_turn"},
    ...
    def test_wrap_stream_emits_log_on_completion_web_proxy(self):
        ...
        context = _make_context(
            messages_api_request=_make_request(stream=True),
            collected_message=_make_collected_message(input_tokens=300, output_tokens=120),
        )
        context.metadata["request_start_time"] = time.monotonic()
    ...
    def test_wrap_stream_emits_log_on_error(self):
        ...
        with patch(PATCH_LOGGER) as mock_logger:
            with self.assertRaises(RuntimeError):
                asyncio.run(_collect_stream(self.processor._wrap_stream(_error_iter(), context)))
    ...
    def test_wrap_stream_extracts_usage_across_multiple_chunks(self):
        start = _make_sse_bytes("message_start", {
            "type": "message_start",
            "message": {"id": "msg_1", "usage": {"input_tokens": 50, "output_tokens": 1}},
        })
        filler = _make_sse_bytes("content_block_delta", {
...
class TestExtractUsageFromSSE(unittest.TestCase):
    ...
    def test_extract_input_tokens_from_message_start(self):
        chunk = _make_sse_bytes("message_start", {
            "type": "message_start",
            "message": {"id": "msg_1", "usage": {"input_tokens": 150, "output_tokens": 1}},
        })
    ...
    def test_extract_cache_tokens_from_message_start(self):
        chunk = _make_sse_bytes("message_start", {
            ...
            "message": {"id": "msg_1", "usage": {
                "input_tokens": 150, "output_tokens": 1,
    ...
    def test_extract_output_tokens_from_message_delta(self):
        chunk = _make_sse_bytes("message_delta", {
            "type": "message_delta",
            "delta": {"stop_reason": "end_turn"},
    ...
    def test_non_usage_events_ignored(self):
        chunk = _make_sse_bytes("content_block_delta", {
            "type": "content_block_delta",
            "delta": {"type": "text_delta", "text": "Hello"},
        })
        usage = {"input_tokens": 0, "output_tokens": 0, "cache_read_input_tokens": 0, "cache_creation_input_tokens": 0}
